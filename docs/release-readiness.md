...
| Sanitized public Banking result package | BUILDER/VERIFIER PASS; MISSING RESULTS | Fail-closed content-addressed builder, independent public verifier, Secret/PII gate, idempotence/tamper/conflict tests, and blocked Attempt `ATT_E332E57709E74D73A9E7A2A87F7B2C36`; no release directory exists | Run builder only after final R2 seal; then verify exact package and private CI attestation |
...
| Secret and direct-PII audit | PASS for current intended public tree | `scripts/audit_public_tree.py`: 195 text files, 3 secret + 3 PII rules, zero findings | Rerun after the sanitized real-results package is added |
